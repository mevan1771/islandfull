...
export function MapFilterBar({ activeCategory, onCategoryChange, isTourSelected, dynamicCategories = [], currentVertical, onVerticalChange }: MapFilterBarProps) {
  ...
  return (
    <div className={`absolute bottom-24 md:bottom-8 pb-[env(safe-area-inset-bottom)] left-4 right-4 md:left-1/2 md:right-auto md:-translate-x-1/2 md:w-max md:max-w-2xl z-40 transition-all duration-500 ease-out ${isTourSelected ? 'translate-y-[150%] opacity-0 pointer-events-none' : 'translate-y-0 opacity-100'}`}>
