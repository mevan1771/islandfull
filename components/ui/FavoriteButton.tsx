...
export function FavoriteButton({ activityId, className, variant = 'overlay' }: FavoriteButtonProps) {
  ...
  const inlineClasses = "relative w-10 h-10 flex items-center justify-center rounded-full bg-black shadow-sm shrink-0"
  ...
  if (!isHydrated) {
    return (
      <button className={twMerge(defaultClasses, "opacity-50 cursor-default", className)}>
        <Heart className={twMerge("w-4 h-4 sm:w-5 sm:h-5", variant === 'overlay' ? "drop-shadow-md text-white/90" : "text-white")} />
      </button>
    )
  }
      ...
      <Heart
        className={twMerge(
          ...
          variant === 'overlay' ? (
            ...
            isSaved ? "fill-rose-500 text-rose-500" : "text-white hover:text-rose-400"
          )
        )}
      />
    </button>
